...
function renderParkFishing(){
  ...
  target.innerHTML=rows.map(p=>`
    <article class="park-permit-row full-excel">
      ...
      <div class="permit-money">${new Intl.NumberFormat("fr-FR").format(p.initialPrice)} $</div>
      ...
      <div><span class="permit-status ${parkFishingStatusClass(p.status)}"><i></i>${escapeHtml(p.sourceStatus||p.status)}</span></div>
    </article>
  `).join("");
